...
<div id='form' class="main-page left" style='border-radius:10px; overflow: hidden; box-shadow: 0 0 20px rgba(0,0,0,.2);'>
					...
					<div id="writecomment">
						<form action="<?php echo base_url(); ?>konsultasi/reply" method="POST" onSubmit="return validasi(this)" id="form_komentar">
							...
							<?php if ($this->uri->segment(3) == ''){ ?>
								<p><input type="submit" style='border-radius:10px; overflow: hidden;' name="submit" class="styled-button" value="Kirimkan Pertanyaan" onclick="return confirm('Yakin ingin mengirimkan pertanyaan ini ?')"/></p>
							<?php }else{ ?>
								<p><input type="submit" style='border-radius:10px; overflow: hidden;' name="submit" class="styled-button" value="Kirimkan Balasan" onclick="return confirm('Kirimkan Sebagai Balasan Pesan terpilih?')"/></p>
							<?php } ?>
						</form>
					</div>
				</div>
